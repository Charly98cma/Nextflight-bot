...
def start_Command(update, context):
    ...
    """
    logger.info('User {} starts a new conversation'
...
def location(update, context):
    ...
    logger.info('User {} timezone is {}'
                .format(update.message.from_user.first_name, tz))
...
def skip_location(update, context):
    ...
    """
    logger.info('User {} didn\'t shared location'
                .format(update.message.from_user.first_name))
...
def help_Command(update, context):
    ...
    """
    logger.info('User {} request the list of commands'
                .format(update.message.from_user.first_name))
...
def nextflight_Command(update, context):
    ...
    """
    logger.info('User {} request next space flight info'
                .format(update.message.from_user.first_name))
...
def nextevent_Command(update, context):
    ...
    """
    logger.info('User {} request info of next event'
                .format(update.message.from_user.first_name))
...
def unknown_Command(update, context):
    ...
    """
    logger.info('User {} send an unknown command {}'
                .format(update.message.from_user.first_name, update.message.text))
...
def main():
    try:
        NF_TOKEN = open('token.txt', 'r').readline().strip()
    except FileNotFoundError:
        print("Environment variable 'NF_TOKEN' not defined.", file=sys_stderr)
        exit(1)

    updater = Updater(token=NF_TOKEN, use_context=True)
